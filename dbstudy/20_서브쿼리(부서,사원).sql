...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE LOCATION = (SELECT LOCATION
                     FROM DEPARTMENT
                    WHERE DEPT_NO = 1);  -- DEPT_NO는 PK이모르 단일 행 서브쿼리
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
  FROM EMPLOYEE
 WHERE POSITION IN(SELECT POSITION      -- 자음 행 서브쿼리의 동등 비교는 IN 연산으로 수행해야 함
                     FROM EMPLOYEE
                    WHERE DEPART = 2);  -- DEPART가 PK.UNIQUE가 아니기 때문에 다중 행 서브쿼리

-- TIP) 단일 행/다중 행 산관 없이 동등 비교는 IN 연산으로 수행가능
...
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE, SALARY
 ...
 WHERE DEPART IN(SELECT DEPT_NO
                   FROM DEPARTMENT
                  WHERE DEPT_NAME = '영업부');  -- DEPT_NAME가 PK/UNIQUE가 아나기 때문에 다중 행 서브쿼리
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE DEPT_NO IN(SELECT DEPART
                    FROM EMPLOYEE
                   WHERE POSITION = '과장');   -- DEPT_NAME가 PK/UNIQUE가 아나기 때문에 다중 행 서브쿼리
...
--    사원번호(EMP_NO), 사원명(NAME), 부서번호(DEPTART), 부서명(DEPT_NAME) 조회
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWID = 'AAAFENAABAAALDBAAC';

-- ROWNUM
-- 1. ROWNUM은 1을 포함하는 범위는 조건으로 사용할 수 있음
-- 2. ROWNUM은 1을 포한하지 않는 범위는 조건으로 사용할 수 없음
SELECT EMP_NO, NAME
  FROM EMPLOYEE
 WHERE ROWNUM = 1;  -- 조회가능
...
CREATE TABLE EMPLOYEE2
    AS (SELECT EMP_NO, NAME, GENDER, POSITION, HIRE_DATE, SALARY
          FROM EMPLOYEE);
