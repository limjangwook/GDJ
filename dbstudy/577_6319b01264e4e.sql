...
SELECT FLOOR(SYSDATE - TO_DATE(HIREDATE)) FROM PROFESSOR;
...
-- 7. 모든 학생의 학생명, 학과명, 담당교수명을 조회하시오. (10점)
SELECT S.SNAME, D.DNAME, P.PNAME
  FROM PROFESSOR P, DEPARTMENT D, STUDENT S
 WHERE P.PROFNO = D.PROFNO
   AND D.DEPTNO = S.DEPTNO;

-- 8. 모든 학과의 학과별 학생수를 조회하시오. 소속 학생이 없으면 0으로 조회하시오. (10점)
SELECT D.DNAME AS 학과명, COUNT(S.STUDNO) AS 학생수
  FROM DEPARTMENT D, STUDENT S
 WHERE D.DEPTNO = S.DEPTNO
 GROUP BY D.DEPTNO, D.DNAME;

-- 9. 모든 교수들의 연봉순위, 교수명, 연봉을 조회하시오. 연봉순으로 조회하시오. (10점)
SELECT D.DNAME AS 학과명, FLOOR(AVG(P.SAL)) AS 평균급여
  FROM PROFESSOR P, DEPARTMENT D, STUDENT S
 WHERE P.PROFNO = D.PROFNO
   AND D.DEPTNO = S.DEPTNO
 GROUP BY D.DEPTNO, D.DNAME;
...
SELECT GRADE AS 학년, ROUND(AVG(HEIGHT), 1) AS 평균키,
                      ROUND(AVG(HEIGHT), 1) AS 평균몸무게
  FROM STUDENT
 GROUP BY GRADE
 ORDER BY GRADE;
...
RETURN NUMBER
IS
    STUDENT_NO STUDENT.STUDNO%TYPE;    
BEGIN
    SELECT STUDNO
      INTO STUDENT_NO
      FROM STUDENT
