/*
    ...
    4. 서버머세지 출력을 위해서 최초 1회 설정이 필요(디폴트 SET SERVEROUTOUT OFF)
        SET SERVEROUTPUT ON;
*/
...
BEGIN
    ...
    ELSE
        GRADE := 'F';
    END IF;
    DBMS_OUTPUT.PUT_LINE(SCORE || '점은 ' || GRADE || '학점입나다.');
END;

-- 3) EMPLOYEE_ID가 150인 사원의 연봉 참조하여
...
DECLARE
    N NUMBER;
    MODULAR NUMBER;
    RESULT VARCHAR2(10 BYTE); -- 2로 나눈 나머지 값을 저장
BEGIN
    FOR N IN 1..10 LOOP
        SELECT MOD(N, 2)
          INTO MODULAR
