...
-- 부서 테이블에 행(ROW) 삽입
-- 부모 테이블(관계에서 PK를 가진 테이블)에 먼저 삽입해야함
...
INSERT INTO
    EMPLOYEE
VALUES
    (1002, '김민서', 1, '사원', 'F', '17/08/01', 2000000);
INSERT INTO
    EMPLOYEE
VALUES
...
-- 테이블 삭제
-- 1. 부서번호가 4인 부서를 삭제
-- 부서번호가 4인 행(ROW)을 참조한느 사원이 없으므로 정상 삭제
DELETE
  FROM DEPARTMENT
 WHERE DEPT_NO = 4;
 
-- 2. 부서번호가 1인 부서를 삭제
-- 외래키 옵션으로ON DELETE SET NULL 처리를 하였기 때문에
