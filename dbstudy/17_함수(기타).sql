...
--    2) RANK() OVER(ORDER BY 순위구할칼럼 DESC) : 내림차누 순위, 높은 값이 1등
...
-- 2) EMPLOYEES 테이블의 사원 정보르 입사순으로 조회하기
--    먼저 입사한 사원이 1등
SELECT
...
--    그룹화작업을 수행하므로 집계함수(그룹함수)와 함계 사용이 가능함
SELECT
...
-- EEMPLOYEE_ID, HIRE_DATE(YYYY-MM-DD), 근무개월수, 퇴직금정산유무
-- 퇴직금정산대상 : 근무개월수가 240개월 이상이며 '정산대상', 아니면 빈 문자열
SELECT
        EMPLOYEE_ID AS 사원번호
      , TO_CHAR(HIRE_DATE, 'YYYY-MM-MM-DD') AS 입사일
