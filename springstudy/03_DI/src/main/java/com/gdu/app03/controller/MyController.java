...
public class MyController {
	...
	*/
	...
	/*
		...
		1. @Inject
			1) 타입(class)이 일치히는 Bean을 가져오는 애너테이션
			2) 동일한 타입이 여러 개 잇는 경우 @Qualifier를 이용해서 Bean을 식별
		2. @Resource
			1) 이름(id)이    일치히는 Bean을 가져오는 애너테이션
		3. @Autowired
			1) 타입(class)이 일치히는 Bean을 가져오는 애너테이션
	...
	*/
	...
	// 2. 생성장를 이용해 Bean 가져오기
	/*
	private Board board;
	
	@Autowired  // 생성자에는 @Autowired를 생략할 수 있다.
	public  MyController(Board board) {  // 매개변수 Board board로 타입이 Board인 Bean을 
		super();
		this.board = board;
	...
	*/
	...
	public void setBoard(Board board) {  // 매개변수 Board board로 Board 타입의 Bean을 Container에거 가져오거라
		this.board = board;
	}
	*/
