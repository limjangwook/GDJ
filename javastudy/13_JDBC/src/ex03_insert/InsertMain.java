...
public class InsertMain {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 쿼리문에 포함된 ?에 변수 전달하기
			// 쿼리문에 작성된 ?의 순선대로 명시하며 값을 채워줌
			ps.setString(1, board.getTitle());    // 1번째 ?에 board.getTitle() 전달하기(setStirng : title이 String이므로)
			ps.setString(2, board.getContent());  // 2번째 ?에 board.getcContent() 전달하기 (setString : content가 String이므로)
			
			// INSERT문의 실행
			// 1. executeUpdate() 메소드 사용하기
			// 2. execyteUpdate() 메소드의 반환값은 int 타입
			// 3. 반환값
			// 		1) 1이 반환되는 경우 : 1개의 행이 INSERT되엇다는 의미. 성공을 의미함
			// 		2) 0이 반환되는 경우 : 0개의 행이 INSERT되엇다는 의미. 실패을 의미함
