...
public class Main {
	...
	public static void main(String[] args) {
		...
		while(unit1.isAlive() && unit2.isAlive()) {  // while(unit1.getEnergy() > 0 && unit2.get() > 0)
			if(myTurn) {
				System.out.println(unit1.getName() + "의 공격!");
				unit1.attack(unit2);  // unit1이 unit2를 공격한다. // (탱크는 10%확률로 상대를 한 번에 죽임)
			}else {
				System.out.println(unit2.getName() + "의 공격!");  // (마린은 40%확률로 상대를 한 번에 죽임)
				unit2.attack(unit1);  // unit2이 unit1을 공격한다.
			}
